...
def run(params, rig):
    for cat in params['spaces']:
        ...
        target_rotation = cmds.xform(target, q=True, ws=True, ro=True)
        cmds.xform(apply_space_node, ws=True, t=target_position)
        cmds.xform(apply_space_node, ws=True, ro=target_rotation)
        ...
        sc = list()
        weightList = list()
        cnst_type = int(cat['constraintType'])

        if cnst_type == 0:# parentConstraint
            cnst = cmds.parentConstraint(space_nodes, apply_space_node, mo=True)[0]
            sc = cmds.scaleConstraint(space_nodes, apply_space_node, mo=True)[0]
            weightList = cmds.parentConstraint(cnst, query=True, weightAliasList=True)

        elif cnst_type == 1:# pointConstraint
            cnst = cmds.pointConstraint(space_nodes, apply_space_node, mo=True)[0]
            sc = cmds.scaleConstraint(space_nodes, apply_space_node, mo=True)[0]
            weightList = cmds.pointConstraint(cnst, query=True, weightAliasList=True)

        elif cnst_type == 2:  # orientConstraint
...
def create_src_space(space_node, tgt):
    """Create a copy of tgt node which is constrained to space node"""
    tgt_space_node = cmds.duplicate(tgt, po=True)[0]
    all_constraint(src, tgt)



def all_constraint(src, tgt):

    if cmds.objExists(src) and cmds.objExists(tgt):
        cmds.parentConstraint(src, tgt)
