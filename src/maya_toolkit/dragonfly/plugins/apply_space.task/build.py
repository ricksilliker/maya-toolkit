...
def run(params, rig):
    for cat in params['spaces']:
        ...
        pc = cmds.parentConstraint(space_nodes, apply_space_node, mo=True)[0]
        sc = cmds.scaleConstraint(space_nodes, apply_space_node, mo=True)[0]

        for index, spaceNode in enumerate(space_nodes):
            ...
            cmds.connectAttr('{}.outColorR'.format(condition_node), '{}.target[{}].targetWeight'.format(pc, index), force=True)
            cmds.connectAttr('{}.outColorR'.format(condition_node), '{}.target[{}].targetWeight'.format(sc, index), force=True)
