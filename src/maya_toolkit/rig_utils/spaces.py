...
def set_space_name(node, space_name, space_attr="space", key=True, keyPrevious=True):
...
def cycleSpaceUI():
    ...
    if sel:
        for item in sel:
            if cmds.attributeQuery("space", node=item, exists=True):
                result = cmds.confirmDialog(title='Confirm',
                                            ...
                                            dismissString='No')
                if 'Yes' in result:
                    cycleSpace(key=True, keyPrevious=True)

                elif 'No' in result:
                    cycleSpace(key=False, keyPrevious=False)

                else:
                    pass
            else:
                LOG.warning("No space attribute on {}, skipping...".format(item))


def cycleSpace(key=True, keyPrevious=True):
    ...
    if selection:
        for ctrl in selection:
            ...
            spaceAttrName = ''
            if cmds.attributeQuery('space', exists=True, node=ctrl):
                spaceAttrName = 'space'
                ...
                LOG.error('Cannot find space attribute on selected control!')
                return

            curFrame = cmds.currentTime(q=True)
            curSpace = cmds.getAttr(ctrl + '.' + SPACEATTR)
        ...
        LOG.error('Nothing selected, a control with a Space attribute and try again!')
